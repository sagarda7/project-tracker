...
export function StatusUpdate({ projectId, currentStatus }: { projectId: string; currentStatus: ProjectStatus }) {
  ...
  async function onSubmit(values: StatusFormValues) {
    setServerError(null);
    const result = await updateProjectStatusAction(projectId, values);
    if (!result.success) {
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={t("projects.updateStatus")}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
          ...
          <div>
            <Label htmlFor="comment">{t("projects.statusComment")}</Label>
